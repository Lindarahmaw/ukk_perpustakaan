...
<body>
	...
	***********************************-->
	<div id="main-wrapper">
		...
		<div class="nav-header">
			<a href="index.html" class="brand-logo">
				...
				<div class="brand-title">
					...
					<span class="brand-sub-title">@perpusdigital</span>
				</div>
			</a>
		...
		<div class="dlabnav">
			<div class="dlabnav-scroll">
				<ul class="metismenu" id="menu">
					...
					<li><a class="has-arrow " href="<?= base_url('Buku_fiksi/') ?>" aria-expanded="false">
							<i class="fa-solid fa-book"></i>
							<span class="nav-text">Buku</span>
						</a>
						<ul aria-expanded="false">
							<li><a href="<?= base_url('buku_fiksi/') ?>">Buku Fiksi</a></li>
							<li><a href="<?= base_url('buku_nonfiksi/') ?>">Buku Nonfiksi</a></li>
							<li><a href="<?= base_url('buku_pendidikan/') ?>">Buku Pendidikan</a></li>

						</ul>
					</li>

					<li><a href="<?= base_url('List_pinjaman/') ?>" class="" aria-expanded="false">
							<i class="fa-solid fa-list"></i>
							<span class="nav-text">List Pinjaman</span>
						</a>
					</li>
					<!-- <li><a class="has-arrow " href="javascript:void()" aria-expanded="false">
		...
		***********************************-->
		<div class="content-body">
			...
			<div class="row">
				<div class="col-xl-12">
						...
						<?php foreach ($datas as $data): ?>
							<div class="col-xl-3 col-xxl-4 col-lg-4 col-md-6 col-sm-6 items">
								<div class="card contact-bx item-content">
									...
									<div class="card-body user-profile buku-item">
										<div class="image-bx">
											<img src="<?= $data['img'] ?>" data-src="images/contacts/Untitled-3.jpg" alt="">

										</div>
										<div class="media-body user-meta-info">
											<h6 class="fs-18 font-w600 my-1 buku-name"><a href="app-profile.html"
													class="text-black user-name" data-name="Alan Green">
													<?= $data['judul'] ?></a></h6>
													<p class="fs-14 mb-3 user-work" data-occupation="UI Designer">
												<?= $data['penulis'] ?>
											</p>
											<p class="fs-14 mb-3 user-work" data-occupation="UI Designer">ISBN
												<?= $data['code'] ?>
											</p>
											<p class="fs-14 mb-3 user-work" data-occupation="UI Designer">Stock
												<?= $data['jumlah'] ?>
											</p>
											<ul>
												<li><a href="javascript:void(0);" id="openModal" data-bs-toggle="modal"
														data-bs-target="#exampleModal<?= $data['buku_id'] ?>">
														<i class="far fa-comment-alt"></i></a>
												</li>

												<div class="container mt-5">
													<a href="javascript:void(0);" class="btn btn-primary btn-rounded fs-18"
														data-bs-toggle="modal"
														data-bs-target="#pinjam<?= $data['buku_id'] ?>">
														+ Pinjam
													</a>
												</div>
												<div class="container mt-5">
					...
					<?php foreach ($datas as $data): ?>
						<div class="modal fade" id="exampleModal<?= $data['buku_id'] ?>" tabindex="-1"
							aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										<h1 class="modal-title fs-5" id="exampleModalLabel">Deskripsi Buku</h1>
									</div>
									<div class="modal-body">
										...
										*****
										Aku Kalah, Aku Merindukanmu! Adalah ungkapan hati dari seseorang yang terpaksa berpisah dengan kekasih hatinya namun belum siap dalam melepaskan. Aku yang sudah terbiasa dengan kehadiran sang kekasih di hidupnya, merasakan kekosongan yang sangat hebat akibat perpisahan mereka. Meski begitu, Aku tidak bisa melakukan apa pun untuk menuntaskan rasa sakit sekaligus rindu itu, sebab sang kekasih hati diketahui sudah memiliki orang baru di hidupnya. -->
									</div>
					...
					<?php foreach ($datas as $data): ?>
						<div class="modal fade" id="reviewModal" <?= $data['buku_id'] ?> tabindex="-1"
							aria-labelledby="reviewModalLabel" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="reviewModalLabel">Beri Ulasan & Rating Buku</h5>
										<button type="button" class="btn-close" data-bs-dismiss="modal"
											aria-label="Close"></button>
									</div>
									<div class="modal-body">
										<input type="hidden" id="bukuId" value="<?= $data['buku_id'] ?>">
										<!-- Menyimpan ID buku -->
										...
										<div class="mt-2">
											<strong>Rating: <span id="ratingValue">0</span></strong>
										</div>
									</div>
									<script
									...
									<!-- <script>
											function submitReview() {
												...
												let stars = document.querySelectorAll('.stars i.text-warning').length;

												if (bookTitle === "" || review === "") {
													alert("Harap isi judul buku dan ulasan.");
													return;
												}

												let reviewContainer = document.getElementById('reviewList');
												let reviewItem = document.createElement('div');
												reviewItem.classList.add('border', 'p-2', 'mb-2', 'bg-white', 'rounded');
												reviewItem.innerHTML = `<strong>${bookTitle}</strong> (${stars} ⭐)<br>${review}`;
												reviewContainer.prepend(reviewItem);

												document.getElementById('bookTitle').value = "";
												document.getElementById('reviewText').value = "";
												document.querySelectorAll('.stars i').forEach(s => s.classList.remove('text-warning'));
											...
											// 			document.querySelectorAll('.stars i')[i].classList.add('text-warning');
											// 		}
											// 	});
											// });
										</script> -->
					...
					<?php foreach ($datas as $data): ?>
						...
						<div class="modal fade" id="pinjam<?= $data['buku_id'] ?>" tabindex="-1"
							aria-labelledby="pinjamModalLabel<?= $data['buku_id'] ?>" aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										<h5 class="modal-title" id="pinjamModalLabel<?= $data['buku_id'] ?>">
									...
									<div class="modal-body">
										<form action="<?= base_url('Pinjam/') ?>" method="post">
											...
											<div class="mb-3">
												<label for="tanggalKembali" class="form-label">Tanggal
													Pengembalian</label>
												<input type="date" class="form-control" name="tanggal_pengembalian"
													required>
											</div>
											<div class="mb-3">
												<label for="peminjam_id" class="form-label">Peminjam ID</label>
												<input type="hidden" class="form-control" name="peminjam_id"
													value="<?= $this->session->userdata('peminjam_id') ?>" required>
											</div>
											<div class="mb-3">
												<label for="buku_id" class="form-label">Buku ID</label>
												<input type="hidden" class="form-control" name="buku_id"
													value="<?= $data['buku_id'] ?>" required readonly>
											</div>
											<div class="modal-footer">
												<button type="button" class="btn btn-secondary"
													data-bs-dismiss="modal">Batal</button>
		...
		<!-- <div class="container mt-5">
			<a href="javascript:void(0);" class="btn btn-primary btn-rounded fs-18" data-bs-toggle="modal"
				data-bs-target="#pinjamModal">+ Pinjam</a>
		</div> -->
			...
			document.addEventListener('DOMContentLoaded', function () {
				...
				searchInput.addEventListener('keyup', function () {
					...
					bukuItems.forEach(item => {
						const bukuNameElement = item.querySelector('.buku-name');

						if (bukuNameElement) {
							const bukuName = bukuNameElement.textContent.toLowerCase().trim();

							if (bukuName.includes(filter)) {
								item.style.visibility = 'visible'; // Tampilkan elemen
								item.style.position = 'relative';
								item.style.display = 'block'; // Pastikan elemen tetap pada tempatnya
							} else {
								item.style.visibility = 'hidden'; // Sembunyikan tanpa mengubah tata letak
		...
		<script>
			...
			function submitReview() {
				const bukuId = document.getElementById('bukuId').value; // Ambil ID buku
				const reviewText = document.getElementById('reviewText').value; // Ambil teks ulasan
				const rating = document.getElementById('ratingValue').innerText; // Ambil rating dari elemen
				console.log('Buku ID:', bukuId);
				console.log('Review Text:', reviewText);
				console.log('Rating:', rating);
				// Cek apakah semua field terisi
				if (bukuId && reviewText && rating > 0) {
					// Kirim data ke server menggunakan AJAX
					$.ajax({
						url: '<?= base_url('ulasan_add/') ?>', // Ganti dengan URL yang sesuai
						type: 'POST',
						data: {
							buku_id: bukuId,
							rating: rating,
						...
						success: function (response) {
							...
							const ulasanList = document.querySelector('.list-group');
							const newUlasan = document.createElement('li');
							newUlasan.classList.add('list-group-item');
							newUlasan.innerHTML = `<strong>Rating: ${rating}</strong><p>${reviewText}</p><small>Ditambahkan baru saja</small>`;
							ulasanList.prepend(newUlasan); // Menambahkan ulasan baru di atas daftar
						},
					...
					alert('Silakan lengkapi semua field dan pilih rating.'); // Pesan kesalahan
				}
			}				
		</script>
	...
	***********************************-->
	...
	<script src="<?= base_url('assets/dashboard') ?>/vendor/jquery-nice-select/js/jquery.nice-select.min.js"></script>
	...
	<script src="<?= base_url('assets/dashboard') ?>/js/dlabnav-init.js"></script>
	<script src="<?= base_url('assets/dashboard') ?>/js/demo.js"></script>
	<script src="<?= base_url('assets/dashboard') ?>/js/styleSwitcher.js"></script>
